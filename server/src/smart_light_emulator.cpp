...
SmartLightEmulator::SmartLightEmulator(const std::string& config_path)
    : m_config(config_path),
      m_topic("smarthome/living_room/light1")
{
    m_broker_conn = std::make_unique<BrokerConnection>(*this, m_context); // <--- Тепер це працює
}
...
void SmartLightEmulator::run()
{
    ...
    try {
        ...
        std::cerr << "[Emulator] FATAL: " << e.what() << std::endl;
        return;
    }
...
void SmartLightEmulator::onMessageReceived(const std::string& topic, const common::NetMessage& msg) // <--- ЗМІНА
{
    if (topic != m_topic) {
        return; 
    }

    save_log("Отримано команду: " + msg.command);
...
void SmartLightEmulator::publish_status()
{
    ...
    status_msg.command = common::NetMessage::CMD_SET_STATUS; // <--- Немає 'topic'
    status_msg.payload = {
        {"state", m_power_state ? "on" : "off"}
    };
    
    try {
        m_broker_conn->publish(m_topic, status_msg); // <--- 'topic' іде сюди
    } catch (const common::ConnectionError& e) {
        std::cerr << "[Emulator] Помилка відправки: " << e.what() << std::endl;
    }
}
...
void SmartLightEmulator::save_log(const std::string& entry)
{
    try {
        ...
        std::cerr << "[Emulator] Помилка запису логу: " << e.what() << std::endl;
    }
}